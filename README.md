...
3. **Downloading**: When you request an episode download, the backend uses the spreadsheet episode name, release date, and CRC32 to find a matching individual or batch release from the cached One Pace release feed. Candidate releases are verified against Nyaa.si file listings by CRC32 before their magnet links are added to qBittorrent. If a spreadsheet CRC is known to be stale, `backend/release_crc32_overrides.json` can provide a metadata-only CRC correction keyed by season and episode number; these overrides are only used after normal CRC verification fails and the override release date matches the candidate release. qBittorrent file priorities are set so only the requested episode downloads, and once done, the file is hardlinked (or copied) to the Jellyfin media location.
